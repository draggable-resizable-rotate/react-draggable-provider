...
import parseMatrix from './../Graphics/parseMatrix';
import {
  drawRect,
...
interface MouseDownCache {
  ...
  rotateData?: {
    rotateRect: ReturnType<typeof getRotateRect>;
...
export default class Resizable extends React.PureComponent<ResizableProps, State> {
  ...
  getRotateRect(options: {
    ...
    // 获取元素旋转的属性
    const rotateRect = getRotateRect({ ...position, ...size }, rotate);
    const directionStartPoint: Point = rotateRect.point[direction];
    // 记录角点的位置信息
    ...
    // 固定点的位置信息
    const fixedPointPosition = rotateRect.point[RECT_DIRECT[fixedPointIndex]];
    // 对点信息
    const apexAnglePointPosition = rotateRect.point[RECT_DIRECT[apexAnglePointIndex]];
    // 对角的方程
